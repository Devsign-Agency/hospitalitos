...
void main() {
  ...
  setUp(() async {
    book = await _loadTestBook().then(EpubReader.readBook);
  });

  test('generatePackageDocumentCFIComponent packageDocument failed', () async {
    String? result;
    try {
      result = const EpubCfiGenerator().generatePackageDocumentCFIComponent(
  ...
  test('generatePackageDocumentCFIComponent idRef failed', () async {
    ...
    try {
      result = const EpubCfiGenerator().generatePackageDocumentCFIComponent(
          EpubChapter()..Anchor = 'idRef', book.Schema!.Package!);
    } catch (e) {
  ...
  test('generatePackageDocumentCFIComponent success', () async {
    final result = const EpubCfiGenerator().generatePackageDocumentCFIComponent(
        EpubChapter()..Anchor = 'id4', book.Schema!.Package!);
  ...
  test('generateElementCFIComponent failed - startElement is null', () async {
    String? result;
    try {
      result = const EpubCfiGenerator().generateElementCFIComponent(null);
    } catch (e) {
      expect(
        e.toString(),
  ...
  test('generateElementCFIComponent failed - startElement has wrong type',
    ...
    try {
      result =
          const EpubCfiGenerator().generateElementCFIComponent(Comment('comment'));
    } catch (e) {
      expect(
        e.toString(),
  ...
  test('generateElementCFIComponent success', () async {
    final document = EpubCfiReader().chapterDocument(book.Chapters![0])!;
    final node = document.getElementsByTagName('p')[3];

    final result = const EpubCfiGenerator().generateElementCFIComponent(node);
  ...
  test('generateCompleteCFI success', () async {
    final document =
        EpubCfiReader().chapterDocument(book.Chapters![0].SubChapters![1])!;
    final node = document.getElementsByTagName('p')[2];

    final packageDocumentCFIComponent = const EpubCfiGenerator()
        .generatePackageDocumentCFIComponent(
            EpubChapter()..Anchor = 'id3', book.Schema!.Package!);
    final contentDocumentCFIComponent =
        const EpubCfiGenerator().generateElementCFIComponent(node);

    final result = const EpubCfiGenerator().generateCompleteCFI(
